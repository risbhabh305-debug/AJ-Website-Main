    ...
    <div className="container mx-auto max-w-5xl px-4">
      ...
      <div className="flex flex-col gap-12 lg:flex-row">
        ...
        <motion.form className="space-y-4 px-1 py-2 sm:px-2 lg:w-1/2" initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col gap-4 sm:flex-row"><a href="https://wa.me/918097988287" target="_blank" rel="noopener noreferrer" className="touch-manipulation flex-1 rounded-full bg-gold px-4 py-3 text-center font-medium text-primary transition hover:bg-opacity-90">WhatsApp Inquiry</a><a href="tel:+918097988287" className="touch-manipulation flex-1 rounded-full border border-gold px-4 py-3 text-center font-medium text-gold transition hover:bg-gold hover:text-primary">Call Now</a><button type="submit" disabled={submissionStatus === 'sending'} className="touch-manipulation flex-1 rounded-full border border-gold px-4 py-3 text-center font-medium text-gold transition hover:bg-gold hover:text-primary disabled:cursor-wait disabled:opacity-60">{submissionStatus === 'sending' ? 'Sending…' : 'Send Inquiry'}</button></div>
        </motion.form>
      </div>
    </div>
  </section>
